package app.demo.weibotestdemo.activity.picturePicker;
...
public class PicturePickerModel {
    ...
    /**遍历StorageCard找寻所有图片所在的文件夹以及文件夹内部的图片URI*/
    private List<PictureFolderModel> getPictureFolderList() {
        ...
        try {
            cursor = MyApp.getContext().getContentResolver().query(
                    ...
                    new String[]{MediaStore.Images.Media.DATA},//查询的表
                    MediaStore.Images.Media.MIME_TYPE + "=? or " + MediaStore.Images.Media.MIME_TYPE + "=? or " + MediaStore.Images.Media.MIME_TYPE + "=?",//查询的属性
                    new String[]{"image/jpeg", "image/png", "image/jpg"},//具体值
                    MediaStore.Images.Media.DATE_ADDED + " DESC");//时间降序
